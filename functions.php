...
function html_start($title, $refresh = 0, $cacheable = true, $report = false)
{
    ...
    echo '<link rel="shortcut icon" href="public/images/favicon.png" >' . "\n";
 ...
 * @return Filter|int
 */
function html_head($title, $refresh = 0, $cacheable = true, $report = false)
{
    if (PHP_SAPI !== 'cli') {
        if (!$cacheable) {
            // Cache control (as per PHP website)
    ...
    echo '<link href="public/lib/Bootstrap/css/bootstrap.min.css" rel="stylesheet" />';
    echo '<link href="public/lib/Fontawesome/css/all.css" rel="stylesheet">';
    echo '<link href="public/css/styles.css" rel="stylesheet" />';
    echo '<link href="public/css/warriors-style.css" rel="stylesheet" />';
    echo '<link href="public/lib/DataTables/datatables.min.css" rel="stylesheet" />';
    echo '<script src="public/js/get_data.js"></script>';
    ...
    echo '<script src="public/lib/JQuery/jquery-3.5.1.min.js" crossorigin="anonymous"></script>';
    echo '<script src="public/js/detail.js"></script>';
    echo '</head>' . "\n";
 ...
 * @return Filter|int
 */
function html_filter($title, $refresh = 0, $cacheable = true, $report = false)
{
...
function html_body($link_activo, $greylink="")
{
    
    echo '<body class="sb-nav-fixed" onload="updateClock(); setInterval(\'updateClock()\', 1000 )">' . "\n";
    
    html_navbar();
    html_sidenav($link_activo, $greylink);
    echo '<div id="layoutSidenav_content">';
    echo '<main>';
}

function html_end_new()
{
    echo '</main>';
    html_footer();
    include_once __DIR__ . '/components/modal_search.php';
    echo '</div>';
    echo '</div>';
    ...
    echo '<script src="public/lib/Bootstrap/js/bootstrap.min.js" crossorigin="anonymous"></script>' . "\n";
    echo '<script src="public/lib/DataTables/datatables.min.js" type="text/javascript" ></script>';
    echo '<script src="public/js/scripts-warriors.js"></script>' . "\n";
    echo '<script src="public/js/scripts_tables.js" crossorigin="anonymous"></script>';
    echo '<script src="public/js/modal_table_subject.js"></script>';
    echo '<script src="public/js/modal_table_add_WB.js"></script>';
    echo '<script src="public/js/modal_search.js"></script>';
...
function html_navbar()
{
    ...
    echo '<nav class="sb-topnav navbar navbar-expand navbar-dark navbar-warriors">' . "\n";
    echo '<a class="navbar-brand logoicon" href="dashboard.php"><img class="logowr" src="public/images/logonhdr.png"></a>' . "\n";
    echo '<div class="d-none d-md-inline-block form-inline ml-auto mr-0 mr-md-3 my-2 my-md-0 navbar-clock">';
    echo '<a class="" ><span id="clock">&nbsp;</span></a>' . "\n";
    echo '</div>';

    
    // // TODO: Buscador
    echo '<div class="d-none d-md-inline-block form-inline ml-auto mr-md-2">';
    echo '    <div class="d-flex">';
    echo '        <input type="hidden" name="token" value="'.$_SESSION["token"].'">';                    
    echo '        <input id="input-search" placeholder="'.__('jumpmessage03').'" class="form-control form-control-search-wr" id="input-search"/>';
    echo '        <a data-toggle="modal" data-target="#modal_search" class="btn btn-warrior-search" ><i class="fas fa-search"></i></a>';
    echo '    </div>';
    echo '</div>';

    
    echo '<div class="navbar-lang">';
    if (defined('USER_SELECTABLE_LANG')) {
        ...
        if ($langCount > 1) {
            ...
            echo '<select id="langSelect" class="lang" onChange="changeLang()">' . "\n";
    ...
    echo '<ul class="navbar-nav ml-auto ml-md-0">' . "\n";
          echo '<li class="nav-item dropdown">' . "\n";
             echo '<a class="nav-link nav-link-wr" id="userDropdown" href="#" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">' . "\n";
                echo '<i class="fas fa-user fa-fw icon-bar-wr"></i></a>' . "\n";
             echo '<div class="dropdown-menu dropdown-menu-right" aria-labelledby="userDropdown">' . "\n";
                echo '<a class="dropdown-item" href="#">' . $_SESSION['fullname'] . '</a>' . "\n";
                echo '<a class="dropdown-item" href="logout.php">Logout <i class="fas fa-sign-out-alt"></i></a>' . "\n";
             echo '</div>' . "\n";
          echo '</li>' . "\n";
    echo '</ul>' . "\n";
    echo '<button class="btn btn-link btn-sm order-1 order-lg-0" id="sidebarToggle" href="#"><i class="fas fa-bars icon-bar-wr"></i></button>'."\n";
 echo '</nav>' . "\n";
}
...
// 
function html_sidenav($link_activo, $greylink){ 
    echo '<div id="layoutSidenav">' . "\n";
        echo '<div id="layoutSidenav_nav">' . "\n";
        echo '<nav class="sb-sidenav accordion sb-sidenav-dark sb-sidenav-warrior" id="sidenavAccordion">' . "\n";
...
function printColorCodesv2()
{
    echo '<div class="d-flex flex-wrap justify-content-center itypemsg2">';
    
        echo '<div class="d-flex flex-column itypemsg_item2">';
        echo '    <div class="itypemsg_text2 clean">'. __('clean03') .'</div>';
        echo '</div>';

        echo '<div class="d-flex flex-column itypemsg_item2">';
        echo '    <div class="itypemsg_text2 whitelisted">'. __('whitelisted03') .'</div>';
        echo '</div>';

        echo '<div class="d-flex flex-column itypemsg_item2">';
        echo '    <div class="itypemsg_text2 blacklisted">'. __('blacklisted03') .'</div>';
        echo '</div>';

        echo '<div class="d-flex flex-column itypemsg_item2">';
        echo '    <div class="itypemsg_text2 spam">'. __('spam103') .'</div>';
        echo '</div>';

        echo '<div class="d-flex flex-column itypemsg_item2">';
        echo '    <div class="itypemsg_text2 highspam">'. __('highspam03') .'</div>';
        echo '</div>';

        echo '<div class="d-flex flex-column itypemsg_item2">';
        echo '    <div class="itypemsg_text2 infected">'. __('badcontentinfected03') .'</div>';
        echo '</div>';

        echo '<div class="d-flex flex-column itypemsg_item2">';
        echo '    <div class="itypemsg_text2 notscanned">'. __('notverified03') .'</div>';
        echo '</div>';
        ...
        if (get_conf_truefalse('mcpchecks')) {
            ...
            echo '<div class="d-flex flex-column itypemsg_item2">';
            echo '    <div class="itypemsg_text2 mcp">'. __('mcp03') .'</div>';
            echo '</div>';

            echo '<div class="d-flex flex-column itypemsg_item2">';
            echo '    <div class="itypemsg_text2 highmcp">'. __('highmcp03') .'</div>';
            echo '</div>';
        } 
    echo '</div>';
}
...
function printServiceStatus_v2()
{
    // MailScanner running?
    if (!DISTRIBUTED_SETUP) {
        $no = '<span class="yes">&nbsp;' . __('no03') . '&nbsp;</span>' . "\n";
        ...
        echo '     <tr><td>' . __('mailscanner03') . '</td><td align="center">' . $running . '</td><td align="left">' . $procs . '</td></tr>' . "\n";
        ...
        if ($mta === 'msmail') {
            ...
            $procs = count($output) . ' ' . __('procs03');
            echo '    <tr><td>' . ucwords('MTA') . __('colon99') . '</td>'
                . '<td align="center">' . $running . '</td><td align="left">' . $procs . '</td></tr>' . "\n";
...
function printMTAQueue()
{
    ...
    if (get_conf_var('MTA', true) === 'postfix') {
        ...
        if ($inq !== null && $outq !== null) {
            ...
            echo '    <tr><td colspan="2"><a href="postfixmailq_.php">' . __('inbound03') . '</a></td><td align="right">' . $inq . '</td></tr>' . "\n";
            echo '    <tr><td colspan="2"><a href="postfixmailq_.php">' . __('outbound03') . '</a></td><td align="right">' . $outq . '</td></tr>' . "\n";
        }
        ...
        if ($inq !== null && $outq !== null && $inq2 !== null && $outq2 !== null) {
            ...
            echo '    <tr><td colspan="2"><a href="postfixmailq_.php">Postfix ' . __('inbound03') . '</a></td><td align="right">' . $inq2 . '</td></tr>' . "\n";
            echo '    <tr><td colspan="2"><a href="postfixmailq_.php">Postfix ' . __('outbound03') . '</a></td><td align="right">' . $outq2 . '</td></tr>' . "\n";
        }
 ...
 * @return string
 */
function format_spam_report_v2($spamreport)
{
    // Run regex against the MailScanner spamreport picking out the (score=xx, required x, RULES...)
    if (preg_match('/\s\((.+?)\)/i', $spamreport, $sa_rules)) {
        // Get rid of the first match from the array
        ...
        $output_array = array();
        foreach ($sa_rules as $sa_rule) {
            $output_array[] = get_sa_rule_desc_v2($sa_rule);
        }

        // Return the result as an html formatted string
        if (count($output_array) > 0) {
            return '<table width="100%" class="wr-tablereport table-sm table-striped">
                        <tr class="wr-tabletr">
                            <th class="wr-tableheading2">' . __('score03') . '</th>
                            <th class="wr-tableheading2">' . __('matrule03') . '</th>
                            <th class="wr-tableheading2">' . __('description03') . '</th></tr>' . implode(
                    "\n",
                    $output_array
                ) . '</table>' . "\n";
        }
 ...
 * @return string
 */
function get_sa_rule_desc_v2($rule)
{
    // Check if SA scoring is enabled
    $rule_score = '';
    if (preg_match('/^(.+) (.+)$/', $rule, $regs)) {
    ...
    if ($row && $row->rule && $row->rule_desc) {
        return ('<tr>
                    <td class="p-2">' . $rule_score . '</td>
                    <td class="p-2">' . $row->rule . '</td>
                    <td class="p-2">' . $row->rule_desc . '</td>
                </tr>' . "\n");
    }
 ...
 * @param bool $actions
 */
function db_colorised_table_v2($sql, $table_heading = false, $pager = false, $order = false, $operations = false, $actions = false)
{
    require_once __DIR__ . '/lib/pear/Mail/mimeDecode.php';

    // Ordering
    echo '<div id="typemessages">
    <div class="card card-wr">
        <div class="card-header card-header-warriors" id="headingOne">
            
            <button class="btn card-header-label-btn" data-toggle="collapse" data-target="#recentmsg" aria-expanded="true" aria-controls="collapseOne">
                <i class="far fa-list-alt"></i>    
                '. __('typeofmessages03') .'
            </button>
            
        </div>                 
        <div id="recentmsg" class="collapse show" aria-labelledby="headingOne" data-parent="#typemessages">
            <div class="card-body">';
                printColorCodesv2();
    echo'   </div>
            </div>
    ...
    $order = false;
    if ($pager) {
        //Check display page title
       
        $from = generatePager_v2($sql);
    ...
    if ($rows > 0) {     
        ...
        if( isset($_GET['modal_error']) ){
            $msg = str_replace('-', ' ', $_GET['modal_error']);
            echo '
        ...
        if( isset($_GET['modal_success']) ){
            $msg = str_replace('-', ' ', $_GET['modal_success']);
            echo '
        ...
        if ($actions !== false) {
            echo '<form name="actions-add" action="do_message_actions_bw.php" method="POST">' . "\n";
            echo '<input type="hidden" name="token-modal" value="' . $_SESSION['token'] . '">' . "\n";
            echo '<input type="hidden" name="formtoken-modal" value="' . generateFormToken('/do_message_actions_bw.php form token') . '">' . "\n";
            echo'
            <div class="row">
                <div class="col-12 mt-2 mb-2">
                    <a data-toggle="modal" data-target="#msg_notification_modal" class="btn btn-sm btn-wr-red ">' . __('addwlbl07') . '</a>
                </div>
            </div>'; 
        }
        ...
        echo '              <table role="table" id="data-table-50msg" class="table50msg table-dark table-sm" width="100%" cellspacing="0" >' . "\n";
        // Work out which columns to display
        $display = array();
        $orderable = array();
        $fieldname = array();
        ...
        for ($f = 0; $f < $fields; $f++) {
            if ($f === 0 && $operations !== false) {
                ...
                // Set it up not to wrap - tricky way to leach onto the align field
                $align[$f] = 'center" style="white-space:nowrap';
                $fieldname[$f] = __('ops03') . '<br>
                <a href="javascript:SetRadios(\'S\')">' . __('radiospam203') . '</a>&nbsp;&nbsp;&nbsp;
                <a href="javascript:SetRadios(\'H\')">' . __('radioham03') . '</a>&nbsp;&nbsp;&nbsp;
                <a href="javascript:SetRadios(\'F\')">' . __('radioforget03') . '</a>&nbsp;&nbsp;&nbsp;
                <a href="javascript:SetRadios(\'R\')">' . __('radiorelease03') . '</a>';
                continue;
            }
            if ($f === 0 && $actions !== false) {
                // Set up display for operations form elements
                $display[$f] = true;
                $orderable[$f] = false;
                // Set it up not to wrap - tricky way to leach onto the align field
                ...
                $fieldname[$f] = 
                    '<div class="d-flex flex-row">
                        <input id="message_actions_check" class="mt-1" name="message_actions_check" type="checkbox" value="action_check">
                        <div class="btn-t-wb btn-wh" data-toggle="tooltip" data-placement="top" title="'.__('addwl04').'">
                            <i class="fas fa-thumbs-up"></i>
                        </div>
                        <div class="btn-t-wb btn-bl" data-toggle="tooltip" data-placement="top" title="'.__('addbl04').'">
            ...
            // Set up the mysql column to account for operations
            $colnum = $f;
            if ($operations !== false || $actions !== false) {
                $colnum = $f - 1;
            }
        ...
        // Column headings
        echo ' <thead class="thread50msg"role="rowgroup">' . "\n";
        echo ' <tr role="row" class="tr50msg">' . "\n";

        //Se inicializa el contador para la propiedad de prioridad responsiva
        $colcount = 1;
        ...
        for ($f = 0; $f < $fields; $f++) {
            if ($display[$f]) {
                if ($order && $orderable[$f]) {
                    ...
                    $fieldInfo = $sth->fetch_field_direct($colnum);
                    echo '  <th role="columnheader" class="th50msg prioridad-'.$colcount.'">'."\n";
                    echo "  $fieldname[$f] (<a id=\"table50msgorder\" href=\"?orderby=" . $fieldInfo->name
                        . '&amp;orderdir=a' . subtract_multi_get_vars(
                            array('orderby', 'orderdir')
                        ) . '">A</a>/<a id="table50msgorder" href="?orderby=' . $fieldInfo->name
                        . '&amp;orderdir=d' . subtract_multi_get_vars(array('orderby', 'orderdir')) . "\">D</a>)\n";
                    echo "  </th>\n";
                    $colcount++;
                } else {
                    echo '  <th role="columnheader" class="th50msg prioridad-'.$colcount.'">' . $fieldname[$f] . '</th>' . "\n";
                    $colcount++;
                }
            }
        }
        ...
        echo '</thead> 
              <tbody class="tbody50msg">';
        // Rows
        $id = '';
        $jsRadioCheck = '';
        $jsReleaseCheck = '';
        $jsReleaseCheckActions = '';
        ...
        for ($r = 0; $r < $rows; $r++) {    
            ...
            if ($actions !== false) {
                array_unshift(
                    $row,
                    '<div class="d-flex flex-row">
                        <input name="check-'.$id_msg.'" type="checkbox" value="ID-'.$id_msg.'">
                        <button type="submit" name="BTN-w-'.$id_msg.'" value="BTN-w-'.$id_msg.'" class="btn btn-t-wb btn-wh">
                            <i class="fas fa-thumbs-up"></i>
                        </button>
                        <button type="submit" name="BTN-b-'.$id_msg.'" value="BTN-b-'.$id_msg.'" class="btn btn-t-wb btn-bl">
                            <i class="fas fa-thumbs-down"></i>
                        </button>
                    </div>'
                );
            ...
            for ($f = 0; $f < $fields; $f++) {
                ...
                switch ($field->name) {
                    case 'id':
                        ...
                        $id = $row[$f];
                        // Create a link to detail.php
                        $row[$f] = '<a class="table50msgdetailicon p-1" href="detail.php?token=' . $_SESSION['token'] . '&amp;id=' . $row[$f] . '">' . $row[$f] . '</a>' . "\n";
                        break;
                    case 'id2':
                        // Store the id for later use
                        $id = $row[$f];
                        ...
                        $row[$f] = '<a href="detail.php?token=' . $_SESSION['token'] . "&amp;id=$row[$f]\" ><i class=\"fas fa-info-circle\" aria-hidden=\"true\"></i></a>";
                    ...
                    case 'subject':
                        ...
                        if ( strlen($row[$f]) == 0 )
                            $row[$f] = '...';
                        $row[$f] = '<a class="btn-subject" data-toggle="modal" data-id="'.$id.'" data-target="#msg_subject_modal" >'.$row[$f].'</a>';
                        break;
                    case 'isspam':
                        if ($row[$f] === 'Y' || $row[$f] > 0) {
                            $spam = true;
            ...
            switch (true) {
                case $infected:
                    echo '<tr class="infected tr50msg" role="row">' . "\n";
                    break;
                case $whitelisted:
                    echo '<tr class="whitelisted tr50msg" role="row">' . "\n";
                    break;
                case $blacklisted:
                    echo '<tr class="blacklisted tr50msg" role="row">' . "\n";
                    break;
                case $highspam:
                    echo '<tr class="highspam tr50msg" role="row">' . "\n";
                    break;
                case $spam:
                    echo '<tr class="spam tr50msg" role="row">' . "\n";
                    break;
                case $highmcp:
                    echo '<tr class="highmcp tr50msg" role="row">' . "\n";
                    break;
                case $mcp:
                    echo '<tr class="mcp tr50msg" role="row">' . "\n";
                    break;
                default:
                    if (isset($fieldname['mcpsascore']) && $fieldname['mcpsascore'] !== '') {
                        echo '<tr class="mcp tr50msg" role="row">' . "\n";
                    } else {
                        echo '<tr class="tr50msg clean" role="row">' . "\n";
                    }
                    break;
            }
            // Display the rows
            ...
            for ($f = 0; $f < $fields; $f++) {
                if ($display[$f]) {
                    ...
                    if ($align[$f]) {
                        $alignClassAddon = ' align="' . $align[$f] . '"';
                        if ($f === 0) {
                            $alignClassAddon .= ' link-transparent';
                        }
                    }
                    $tooltipAddon = '';
                    if ($tooltips[$f]) {
                    ...
                    echo ' <td role="cell" class="td50msg prioridad-'.$colcount.' text-left'.$alignClassAddon .'"' .$tooltipAddon .'>' . $row[$f] . '</td>' . "\n";
                    $colcount++;
                }
            }
            echo ' </tr>' . "\n";            
        ...
        if($actions !== false){
            ...
            <div class="row">
                <div class="col-12 mt-2 mb-2">
                    <a type="submit" data-toggle="modal" data-target="#msg_notification_modal" class="btn btn-sm btn-wr-red ">' . __('addwlbl07') . ' </a>
                </div>
            </div>';
        ...
        include_once __DIR__ . '/components/modal_subject.php';
        include_once __DIR__ . '/components/modal_notification.php';
 ...
 * @param bool $operations
 */
function db_colorised_table_vDatable($sql, $table_heading = false, $pager = false, $order = false, $operations = false)
{
    require_once __DIR__ . '/lib/pear/Mail/mimeDecode.php';
    ...
    if ($rows > 0) {     
        ...
        echo'            <div class="table-responsive">';
        if ($operations !== false) {
            echo '<table role="table" id="data-table-Messages_v2" class="table50msg  table-sm" width="100%" cellspacing="0" >' . "\n";
        }else{
            echo '<table role="table" id="data-table-Messages" class="table50msg  table-sm" width="100%" cellspacing="0" >' . "\n";     
        }
        // Work out which columns to display
        $display = array();
        $orderable = array();
        ...
        echo ' </tr>' . "\n";
        //echo '<tr class="tr50msg" role="row"><td role="cell" colspan="'.$column_headings.'" class="td50msg Responsivo"></td></tr>';
        ...
        for ($r = 0; $r < $rows; $r++) {
            ...
            for ($f = 0; $f < $fields; $f++) {
                ...
                switch ($field->name) {
                    ...
                    case 'id2':
                        ...
                        $id = $row[$f];
                        // Create a link to detail.php as [<link>]
                        $row[$f] = '<a class="table50msgdetailicon p-1" href="detail.php?token=' . $_SESSION['token'] . "&amp;id=$row[$f]\" ><i class=\"fas fa-info-circle\" aria-hidden=\"true\"></i></a>";
                        break;
                    case 'from_address':
                        $row[$f] = htmlentities($row[$f]);
                        if (FROMTO_MAXLEN > 0) {
            ...
            for ($f = 0; $f < $fields; $f++) {
                if ($display[$f]) {
                    ...
                    echo ' <td role="cell" class="td50msg text-left'.$alignClassAddon .' prioridad-'.$colcount.'" ' . $tooltipAddon .'>' . $row[$f] . '</td>' . "\n";
                    $colcount++;
                }
            }
            echo ' </tr>' . "\n";            
        ...
        if ($operations !== false) {
            ...
            <div class=\"card mb-4\">
                ...
                <div class=\"card-body m-0 p-3\">
                    ...
                    &nbsp; <a href=\"javascript:SetRadios('R')\">" . __('radiorelease03') . '</a>
                    &nbsp; ' . __('or03') . " <a href=\"javascript:SetRadios('C')\">" . __('clear03') . "</p>
                    <p><input type='SUBMIT' class=\"btn-outline-wr-red\" name='SUBMIT' value='" . __('learn03') . "'></p>
                    </form>
                    <p><b>" . __('spam203') . ' &nbsp; <b>' . __('ham03') . ' &nbsp; <b>' . __('forget03') . ' &nbsp; <b>' . __('release03') . '
                </div></div>' . "\n";
        }
...
function dbtable_v2($sql, $title = null, $pager = false, $operations = false, $tabletitle)
{
    /*
    // Query the data
    $sth = dbquery($sql);
    ...
    */
    ...
    if ($pager) {
        ...
        echo '<div class="card card-wr mb-4">
                <div class="card-header card-header-warriors">
                <i class="far fa-list-alt"></i>
                '. $title .'
            </div>
    ...
    if ($rows > 0) {
        echo '<div class="table-responsive table-res-wr">
        <table  role="table" id="'.$tabletitle.'" class="table table-sm table-striped table-wr">
            <thead>'."\n";   
        // Column headings
        ...
        foreach ($sth->fetch_fields() as $field) {
            echo '  <th class="pt-2 pb-2 pl-2">' . $field->name . '</th>' . "\n";
        }
        echo ' </tr>
            </thead>' . "\n";
        // Rows
        ...
        while ($row = $sth->fetch_row()) {
            ...
            for ($f = 0; $f < $fields; $f++) {
                echo '  <td class="pl-2">' . preg_replace("/,([^\s])/", ', $1',
                        $row[$f]) . '</td>' . "\n";
            }
            echo ' </tr>' . "\n";
        }
...
function validateInput($input, $type)
{
    switch ($type) {
        ...
        case 'mimepart':
            ...
            break;
        case 'loginerror':
            if (preg_match('/^(baduser|emptypassword|timeout|pagetimeout|token)$/', $input)) {
                return true;
            }
            break;
        case 'timeout':
...
function printAverageLoad_v2()
{
    ...
    if (!DISTRIBUTED_SETUP && file_exists('/proc/loadavg')) {
        ...
        echo '
        <tr>
            <td align="left" rowspan="3" style="vertical-align : middle;text-align:left;">' . __('loadaverage03') . '&nbsp;</td>
            <td align="left">' . __('1minute03') . '&nbsp;</td>
            <td align="left">' . $la_1m . '</td>
        </tr>
        <tr>
            <td align="left" colspan="1">' . __('5minutes03') . '&nbsp;</td>
            <td align="left">' . $la_5m . '</td>
        </tr>
        <tr>
            <td align="left" colspan="1">' . __('15minutes03') . '&nbsp;</td>
            <td align="left">' . $la_15m . '</td>
        </tr>
        ' . "\n";
    } elseif (!DISTRIBUTED_SETUP && file_exists('/usr/bin/uptime')) {
        $loadavg = shell_exec('/usr/bin/uptime');
...
function print_graph_rep_top_mail_relays($filter)
{ 
    ...
    $graphgenerator->printPieGraph_dashboard(
        'print_graph_rep_top_mail_relays',
        'rep_top_mail_relays.php');
}


function print_graph_rep_top_viruses($filter)
{ 
...
$graphgenerator->printPieGraph_dashboard(
    'print_graph_rep_top_viruses',
    'rep_top_viruses.php');

}

function print_graph_rep_top_senders_by_volumen($filter)
{ 
    $graphgenerator = new GraphGenerator_v2();
    $graphgenerator->sqlQuery = '
     SELECT
    ...
    $graphgenerator->printPieGraph_dashboard(
        'print_graph_rep_top_senders_by_volumen',
        'rep_top_senders_by_volume.php');
}

function print_graph_rep_top_senders_by_quantity($filter)
{ 
    $graphgenerator = new GraphGenerator_v2();
    $graphgenerator->sqlQuery = '
     SELECT
    ...
    $graphgenerator->printPieGraph_dashboard(
        'print_graph_rep_top_senders_by_quantity',
        'rep_top_senders_by_quantity.php');

}

function print_graph_rep_top_recipients_by_quantity($filter)
{
    $graphgenerator = new GraphGenerator_v2();
    $graphgenerator->sqlQuery = '
     SELECT
    ...
    $graphgenerator->printPieGraph_dashboard(
        'print_graph_rep_top_recipients_by_quantity',
        'rep_top_recipients_by_quantity.php');
}

function print_graph_rep_top_recipients_by_volumen($filter)
{ 
    $graphgenerator = new GraphGenerator_v2();
    $graphgenerator->sqlQuery = '
    SELECT
    ...
    $graphgenerator->printPieGraph_dashboard(
        'print_graph_rep_top_recipients_by_volumen',
        'rep_top_recipients_by_volume.php');

}

function print_graph_rep_top_senders_domain_by_quantity($filter){
    ...
    $graphgenerator->printPieGraph_dashboard(
        'print_graph_rep_top_senders_domain_by_quantity',
        'rep_top_sender_domains_by_quantity.php');
}

function print_graph_rep_top_senders_domain_by_volumen($filter){
    // SQL query to find all emails with a virus found
    $graphgenerator = new GraphGenerator_v2();
    ...
    $graphgenerator->printPieGraph_dashboard(
        'print_graph_rep_top_senders_domain_by_volumen',
        'rep_top_sender_domains_by_volume.php');
}

function print_graph_rep_top_recipients_domain_by_volumen($filter){
// SQL query to find all emails with a virus found
$graphgenerator = new GraphGenerator_v2();
...
$graphgenerator->printPieGraph_dashboard(
    'print_graph_rep_top_recipients_domain_by_volumen',
    'rep_top_recipient_domains_by_volume.php');
}

function print_graph_rep_top_recipients_domain_by_quantity($filter){
    // SQL query to find all emails with a virus found
$graphgenerator = new GraphGenerator_v2();
...
$graphgenerator->printPieGraph_dashboard(
    'print_graph_rep_top_recipients_domain_by_quantity',
    'rep_top_recipient_domains_by_quantity.php');
}


function printTodayStatistics_data()
{
    $sql = '
 SELECT
  COUNT(*) AS processed,
...
';
   ...
   $data_toggle = 'data-toggle="tooltip" data-placement="top"';
    
   echo '<div class="d-flex flex-wrap justify-content-center"> ';
   
   echo '   <div class="text-center dash_info_card">';
   echo '   <div class="d-flex flex-column justify-content-center dash-info" ' .$data_toggle. ' title="' .__('processed03'). '"> ';
   echo '         <div class="text-center dash-label-icon">';
   echo '            <i class="fas fa-arrow-circle-down fa-2x"></i>';
   echo '         </div>';
   echo '         <div class="text-center dash-label-info">';
                     echo number_format($data->processed);
                     echo ' - ';
                     echo formatSize($data->size);
   echo '         </div>';
   echo '   </div>';
   echo '   </div>';

   echo '   <div class="text-center dash_info_card">';
   echo '   <div class="d-flex flex-column justify-content-center dash-info" ' .$data_toggle. ' title="' .__('cleans03'). '"> ';
   echo '         <div class="text-center dash-label-icon">';
   echo '            <i class="fas fa-envelope-open-text fa-2x"></i>';
   echo '         </div>';
   echo '         <div class="text-center dash-label-info">';
                     echo number_format($data->clean);
                     echo ' - ';
                     echo $data->cleanpercent.'%';
   echo '         </div>';
   echo '   </div>';
   echo '   </div>';

   echo '   <div class="text-center dash_info_card">';
   echo '   <div class="d-flex flex-column justify-content-center dash-info" ' .$data_toggle. ' title="' .__('viruses03'). '"> ';
   echo '         <div class="text-center dash-label-icon">';
   echo '            <i class="fas fa-bug fa-2x"></i>';
   echo '         </div>';
   echo '         <div class="text-center dash-label-info">';
                     echo number_format($data->viruses);
                     echo ' - ';
                     echo $data->viruspercent.'%';
   echo '         </div>';
   echo '   </div>';
   echo '   </div>';

   echo '   <div class="text-center dash_info_card">';
   echo '   <div class="d-flex flex-column justify-content-center dash-info" ' .$data_toggle. ' title="' .__('topvirus03'). '"> ';
   echo '         <div class="text-center dash-label-icon">';
   echo '            <i class="fas fa-viruses fa-2x"></i>';
   echo '         </div>';
   echo '         <div class="text-center dash-label-info">';
                     echo return_todays_top_virus();
   echo '         </div>';
   echo '   </div>';
   echo '   </div>';

   echo '   <div class="text-center dash_info_card">';
   echo '   <div class="d-flex flex-column justify-content-center dash-info" ' .$data_toggle. ' title="' .__('blockedfiles03'). '"> ';
   echo '         <div class="text-center dash-label-icon">';
   echo '            <i class="fas fa-ban fa-2x"></i>';
   echo '         </div>';
   echo '         <div class="text-center dash-label-info">';
                     echo number_format($data->blockedfiles);
                     echo ' - ';
                     echo $data->blockedfilespercent.'%';
   echo '         </div>';
   echo '   </div>';
   echo '   </div>';
   
   echo '   <div class="text-center dash_info_card">';
   echo '   <div class="d-flex flex-column justify-content-center dash-info" ' .$data_toggle. ' title="' .__('others03'). '"> ';
   echo '         <div class="text-center dash-label-icon">';
   echo '            <i class="fas fa-mail-bulk fa-2x"></i>';
   echo '         </div>';
   echo '         <div class="text-center dash-label-info">';
                     echo number_format($data->otherinfected);
                     echo ' - ';
                     echo $data->otherinfectedpercent.'%';
   echo '         </div>';
   echo '   </div>';
   echo '   </div>';

   echo '   <div class="text-center dash_info_card">';
   echo '   <div class="d-flex flex-column justify-content-center dash-info" ' .$data_toggle. ' title="' .__('spam03'). '"> ';
   echo '         <div class="text-center dash-label-icon">';
   echo '            <i class="fas fa-exclamation-triangle fa-2x"></i>';
   echo '         </div>';
   echo '         <div class="text-center dash-label-info">';
                     echo number_format($data->spam);
                     echo ' - ';
                     echo $data->spampercent.'%';
   echo '         </div>';
   echo '   </div>';
   echo '   </div>';

   echo '   <div class="text-center dash_info_card">';
   echo '   <div class="d-flex flex-column justify-content-center dash-info" ' .$data_toggle. ' title="' .__('hscospam03'). '"> ';
   echo '         <div class="text-center dash-label-icon">';
   echo '            <i class="fas fa-thermometer-three-quarters fa-2x"></i>';
   echo '         </div>';
   echo '         <div class="text-center dash-label-info">';
                     echo number_format($data->highspam);
                     echo ' - ';
                     echo $data->highspampercent.'%';
   echo '         </div>';
   echo '   </div>';
   echo '   </div>';
...
function printMTAQueue_v2()
{
    ...
    if (get_conf_var('MTA', true) === 'postfix') {
        ...
        if ($inq !== null && $outq !== null) {
            echo '    <tr><td colspan="2"><a class="btn btn-block btn_info btn-sm" href="postfixmailq_.php">' . __('inbound03') . '</a></td><td align="right" class="bg-danger">' . $inq . '</td></tr>' . "\n";
            echo '    <tr><td colspan="2"><a class="btn btn-block btn_info btn-sm" href="postfixmailq_.php">' . __('outbound03') . '</a></td><td align="right">' . $outq . '</td></tr>' . "\n";
        }
    } elseif (get_conf_var('MTA', true) === 'msmail') {
        $incomingdir = get_conf_var('incomingqueuedir', true);
        $outgoingdir = get_conf_var('outgoingqueuedir', true);
        ...
        if ($inq !== null && $outq !== null && $inq2 !== null && $outq2 !== null) {  
            echo '<tr>';
               echo '<td class="table_td_info_a">';  
                  echo '<a class="btn btn-block btn_info btn-sm" href="msmailq.php">Milter ' . __('inbound03') . '</a>';
               echo '</td>';  
               echo '<td class="table_td_info_value">';  
                  echo $inq;
               echo '</td>';  
            echo '</tr>';  
            ...
            echo '<tr>';
               echo '<td class="table_td_info_a">';  
                  echo '<a class="btn btn-block btn_info btn-sm" href="msmailq.php">Milter ' .  __('outbound03') . '</a>';
               echo '</td>';  
               echo '<td class="table_td_info_value">';  
                  echo $outq;
               echo '</td>';  
            ...
            echo '<tr>';
               echo '<td class="table_td_info_a">';  
                  echo '<a class="btn btn-block btn_info btn-sm" href="postfixmailq_.php">Postfix ' . __('inbound03') . '</a>';
               echo '</td>';  
               echo '<td class="table_td_info_value">';  
                  echo $inq2;
               echo '</td>';  
            ...
            echo '<tr>';
               echo '<td class="table_td_info_a">';  
                  echo '<a class="btn btn-block btn_info btn-sm" href="postfixmailq_.php">Postfix ' . __('outbound03') . '</a>';
               echo '</td>';  
               echo '<td class="table_td_info_value">';  
                  echo $outq2;
               echo '</td>';  
        ...
        echo '    <tr><td colspan="2"><a class="btn btn-block btn_info btn-sm" href="mailq.php?token=' . $_SESSION['token'] . '&amp;queue=inq">' . __('inbound03') . '</a></td><td align="right">' . $inq . '</td></tr>' . "\n";
        echo '    <tr><td colspan="2"><a class="btn btn-block btn_info btn-sm" href="mailq.php?token=' . $_SESSION['token'] . '&amp;queue=outq">' . __('outbound03') . '</a></td><td align="right">' . $outq . '</td></tr>' . "\n";
    }
}

function printMTAQueue_data()
{
    // Display the MTA queue
    // Postfix if mta = postfix
    if (get_conf_var('MTA', true) === 'postfix') {
        ...
        if ($inq !== null && $outq !== null && $inq2 !== null && $outq2 !== null) {  
            ...
            echo '<tr>';
            echo '<td class="table_td_info_a">';
                echo '<a class="btn btn-block btn_info btn-sm" href="msmailq.php">';
                    echo '<div class="d-flex flex-row justify-content-between">';
                    echo '<div>Milter ' . __('inbound03') . '</div>';
                    echo '<div>'.$inq.'</div>';
                    echo '</div>';
                echo '</a>';
            echo '</td>';
            ...
            echo '<td class="table_td_info_a">';
                echo '<a class="btn btn-block btn_info btn-sm" href="msmailq.php">';
                    echo '<div class="d-flex flex-row justify-content-between">';
                    echo '<div>Milter ' . __('outbound03') . '</div>';
                    echo '<div>'.$outq.'</div>';
                    echo '</div>';
                echo '</a>';
            echo '</td>';
            ...
            echo '<tr>';
            echo '<td class="table_td_info_a">';
                echo '<a class="btn btn-block btn_info btn-sm" href="postfixmailq_.php">';
                    echo '<div class="d-flex flex-row justify-content-between">';
                    echo '<div>Postfix ' . __('inbound03') . '</div>';
                    echo '<div>'.$inq2.'</div>';
                    echo '</div>';
                echo '</a>';
            echo '</td>';
            ...
            echo '<td class="table_td_info_a">';
                echo '<a class="btn btn-block btn_info btn-sm" href="postfixmailq_.php">';
                    echo '<div class="d-flex flex-row justify-content-between">';
                    echo '<div>Postfix ' . __('outbound03') . '</div>';
                    echo '<div>'.$outq2.'</div>';
                    echo '</div>';
                echo '</a>';
            echo '</td>';
...
function print_pie_chart_card($charttitle,$idchart,$charturl){
    echo '<div class="col-xl-4 col-lg-4 col-md-6 col-sm-6 col-12">
    <div class="card card-wr">
    <div class="card-header card-header-warriors ">
        <a href="'.$charturl.'" class="piechart-detail-link">
            <i class="fas fa-chart-pie"></i>
            '.$charttitle.'
        </a>
    </div>
    <div  id="body'.$idchart.'"  class="card-body card-body-warriors">        
        <div class="d-flex justify-content-center align-items-center">
            <div id="load'.$idchart.'" class="spinner-border text-danger"  style="width: 4rem; height: 4rem;" role="status">
                <span class="sr-only">Loading...</span>
            </div>
            <canvas id="'.$idchart.'" name="'.$idchart.'" style="display:none;" class="chartjs-render-monitor graph-top"></canvas>
        </div>        
    </div>
    </div>
 </div>';
...
function printFreeDiskSpace_v2(){
   echo '<div class="d-flex flex-column">';
    if (!DISTRIBUTED_SETUP) {
        ...
        foreach (get_disks() as $disk) {
            ...
            $percent .= ' [';
            $percent .= round($free_space / $total_space, 2) * 100;
            $free = round($free_space / $total_space, 2) * 100;
            $used = 100 - $free;
            $percent .= '%] ';
            $percent .= '</span>';
            // if( $disk['mountpoint'] !== '/boot' && $disk['mountpoint'] !== '/tmp'){ 
            if( true ){
               echo '<div class="space_disk d-flex flex-column">';
                  echo '<div class="space_disk_labels d-flex flex-row justify-content-between">';
                     echo '<div class="space_disk_labels_path">' . $disk['mountpoint'] . '</div>';
                     echo '<div class="space_disk_labels_free">' . formatSize($used_space) .'/'. formatSize($total_space) . '</div>';
                  echo '</div>';
                  echo '<div class="progress progress_disk">';
                     
                     echo '<div class="progress-bar progress-bar-striped progress-bar-animated progress-bar-used" role="progressbar" style="width: '.$used.'%;" aria-valuenow="'.$used.'" aria-valuemin="0" aria-valuemax="100">';
                        echo $used.'%';
                     echo '</div>';
                     echo '<div class="progress-bar progress-bar-free" role="progressbar" style="width: '.$free.'%;" aria-valuenow="'.$free.'" aria-valuemin="0" aria-valuemax="100">';
                     echo '</div>';
...
{
   $data_uptime = shell_exec("uptime -p");
   $uptime = substr( $data_uptime, 2);

   echo '<tr class="mt-2">';
   echo '<td>'.__('dashuptime66').'</td>';
   echo '<td colspan="2">'.$uptime.'</td>';
   echo '</tr>';
}
