...
class GraphGenerator_v2
{
    ...
    public function printPieGraph()
    {
        ...
        //create canvas graph
        echo '
        <canvas id="' . $chartId . '" class="chartjs-render-monitor"></canvas>
        
        <link href="public/lib/Chart.js/Chart.min.css" rel="stylesheet">
        <script src="public/lib/Chart.js/Chart.min.js"></script>
        <script src="public/js/Charts-warriors/pieWarriors.js"></script>
        <script>
            COLON = "' . __('colon99') . '";
     ...
     * @return void
     */
    public function printPieGraph_dashboard($chartId,$url)
    {
        if ($this->prepareData_dashboard() === false) {
            return;
        }
        ...
        <div class="card-body card-body-warriors">        
            <div class="d-flex justify-content-center align-items-center">
                <canvas id="' . $chartId . '" class="chartjs-render-monitor graph-top"></canvas>
            </div>        
        </div>
        <div class="card-footer card-footer-warriors">
            <a href="'.$url.'" class="btn btn-wr-red text-white btn-sm">Details</a>
        </div>
        </div>
    </div>';
        //create canvas graph
        ...
        <script>
            COLON = "' . __('colon99') . '";
            printPieGraph_dashboard("' . $chartId .'", {
            chartTitle :  "' . $this->graphTitle . '",
            chartId : "' . $chartId . '",
            chartLabels : ["' . implode('", "', $this->data[$this->graphColumns['labelColumn']]) . '"],
            chartNumericData : [' . implode(', ', $this->data[$this->graphColumns['dataNumericColumn']]) . '],
    ...
    public function printLineGraph()
    {
        ...
        <canvas id="' . $chartId . '" class="lineGraph"></canvas>
        
        
        <link href="public/lib/Chart.js/Chart.min.css" rel="stylesheet">
        <script src="public/lib/Chart.js/Chart.min.js"></script>
        <script src="public/js/Charts-warriors/charts.js"></script>
        <script>
            COLON = "' . __('colon99') . '";
     ...
     * @return void
     */
    public function printLineGraph_dashboard($chartId = 'graph')
    {
        if ($this->types === null) {
            echo("No types defined");
            return;
        ...
        <script>
            COLON = "' . __('colon99') . '";
            printLineGraph_dashboard("' . $chartId . '",  {
            chartTitle : "' . $this->graphTitle . '",
            chartId : "' . $chartId . '",
            chartLabels : ["' . implode('", "', $this->data[$this->graphColumns['labelColumn']]) . '"],
            chartNumericData : [' . $numericData . '],
     ...
     * @return boolean true on success, false on error
     */
    protected function prepareData_dashboard()
    {
        $result = dbquery($this->sqlQuery);
        $this->data = array();
        $this->numResult = $result->num_rows;
    ...
    public function printTable()
    {
        ...
        echo '<div class="card card-wr my-4">
                <div class="card-header card-header-warriors">
        ...
        <table class="table table-sm table-striped table-wr" role="table">'."\n";  
    ...
    public function printTable_v2()
    {
        ...
        echo '          <table class="table table-sm table-striped table-wr" style="width:100%" id="rep_total_mail_by_date" cellspacing="0">';
        echo '    <thead class="">';
        echo '    <tr>';
        foreach ($this->tableColumns as $columnName => $columnTitle) {
            echo '     <th>' . $columnTitle . '</th>' . "\n";
