...
if ($_SESSION['user_type'] !== 'A') {
    ...
    $filter = html_head($pageheader, 0, false, false);
    ...
    echo '<li class="breadcrumb-item title_page_li"><a href="msre_index.php">'.__('rulesetedit29').'</a></li>';
    echo '<li class="breadcrumb-item title_page_li active">'.$short_filename.'</li>';
    echo '</ol>';
...
function Show_Form($status_msg, $short_filename, $file_contents, $CONF_ruleset_keyword)
{
    ...
    echo "\n" . sprintf(__('contentsof55'), $short_filename) ."\n";                        
    echo'   </div>
            <div class="card-body card-body-warriors">';    
    // Display the file contents
    echo '      <pre class="p-2" style="background-color:#EEE;">'.$file_contents.'</pre>';
  ...
  //  echo'<div class="col-md-6">';
    ...
    foreach ($ruleset as $ruleanddesc) {
        ...
        $rule_action_select_html = '<select class=" w-25 mr-2 form-control custom-select w-input-text" name="' . $rule_action_select . '"';
        ...
        $desc_text = array(
            ...
            '<b>' . __('description55') . '</b>&nbsp;&nbsp;<input class="form-control w-input-text" type="text" ' .
   ...
   foreach ($rule_part as $key => $value) {
       ...
       switch (strtolower($key)) {
           case '2and':
               ...
               $checkbox_html = '<input class="mr-2" type="checkbox" name="' . $field_name . '" value="';
               $checkbox_html .= 'and';
               $checkbox_html .= '"';
               if ($value) {
           ...
           case '3and_direction':
               ...
               $select_html = '<select class="form-control custom-select w-input-text" name="' . $field_name . '"';
               if ($rule_disabled) {
           ...
           default:
               ...
               if (strtolower($key) === '99action') {
                   $temp_text = '&nbsp;&nbsp;<b>' . __('action55')
                   . '</b>&nbsp;&nbsp;<input class="form-control w-input-text" type="text" name="' . $field_name . '" value="' . $value . '" size="100"';
                    ...
                    $action_row = $temp_text;
                    break;
               } else {
                   $temp_text = '<input class="form-control w-input-text" type="text" name="' . $field_name . '" value="' . $value . '"';
               }
               if ($rule_disabled || (strtolower($key) === '1target' && strtolower($value) === 'default')) {
                   $temp_text .= ' disabled ';
               }
   ...
   // each other in the list.
        ...
        <div class="card-body card-body-warriors">
        '; 
        echo '<div class="d-flex flex-row align-items-center mb-3" style="background-color:1px solid blue;">' . "\n";
        TR_Extended_v2($desc_text, $tr_param);
        echo '</div>';
        echo '<div class="d-flex flex-row align-items-center mb-3" style="background-color:1px solid green;">' . "\n";
        TR_v2($rule_text, $tr_param);
        echo '</div>';
        echo '<div class="d-flex flex-row align-items-center mb-3" style="background-color:1px solid green;">' . "\n";
        echo $action_row;
        echo '</div>';
   ...
   // echo '</div>';
    ...
    $desc_text = array(
        '' => 'rowspan="3"',
        '<b>' . __('description55') . '</b>&nbsp;&nbsp;
        <input class="form-control w-input-text" type="text" name="' .
        $add_prefix . 'description" value="" size="95">' =>
            'colspan="' . (MSRE_COLUMNS - 1) . '"'
    );
    
    // Direction
    $temp_html = '<b>' . __('conditions55') . '</b>&nbsp;&nbsp;
    <select class="form-control custom-select w-input-text" name="' . $add_prefix .
    ...
    // Target
    $add_rule_text[] = '<input class="form-control w-input-text" type="text" name="' . $add_prefix .
        'target" value="">';
    $add_rule_text[] = '<input class="mr-2" type="checkbox" name="' . $add_prefix .
        'and" value="and"> ' . __('and55');
    $temp_html = '<select class="form-control custom-select w-input-text" name="' . $add_prefix .
   ...
   // echo'<div class="col-md-6">';
    ...
    echo '<input class="btn btn-sm btn-wr-red mb-4" type="submit" name="submit" value="' . __('savevalue55') . '">';
    //echo    '<a href="msre_index.php">' . __('backmsre55') . '</a><br>' . "\n" . '<a href="other.php">' . __('backmw55') . '</a><br>' . "\n";
